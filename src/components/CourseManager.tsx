...
const CourseManager = () => {
  ...
  /* ─── Course cards grid (shared between mobile browse tab & desktop) ─── */
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 font-regular">{course.location || 'Unknown location'}</p>
                  </div>
                </div>
          ...
          <aside ref={panelRef} className="w-full lg:w-96">
            <div className="bg-white p-6 rounded-2xl border border-gray-100">
              <div className="flex items-center mb-4">
                <FlagTriangleRight className="mr-3 text-blue-600" size={20} />
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <div className="bg-white px-5 pb-4 pt-2 rounded-2xl border border-gray-100 flex flex-col flex-1 min-h-0">
        ...
        {/* ─── Desktop: always show search + cards ─── */}
        ...
        <div className="flex flex-col flex-1 min-h-0 lg:hidden">
            ...
            <div className="mb-6 relative" ref={dropdownRef}>
              <form onSubmit={handleCreateCourse} className="flex flex-col gap-3 w-full">
                ...
                <button
                  type="submit"
                  disabled={submitting || Boolean(selectedCourseId)}
                  className={`w-full px-6 py-3 rounded-xl font-bold bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 ${selectedCourseId ? 'cursor-not-allowed' : ''}`}
                >
                  {submitting ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mx-auto" /> : 'Add Course'}
                </button>
              </form>
            </div>
          </div>
          ...
          {/* Browse tab */}
          <div className={`flex-1 min-h-0 flex flex-col ${activeTab === 'browse' ? 'flex' : 'hidden'}`}>
            {loading ? (
              <div className="flex items-center justify-center flex-1">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" />
              </div>
